...
<FigureCompare
  title="生成UIに置き換えられやすいツール・置き換えにくいツール"
  caption="Search Engine Journalの2本目の記事の列挙を整理したもの"
  cols={[
    {
      label: "置き換えられやすい",
      tone: "news",
      sub: "公式の計算式だけで動く",
      points: ["住宅ローン計算機、単位換算、BMI計算、支払い計算", "学習シミュレーション、練習問題"],
    },
    {
      label: "置き換えにくい",
      tone: "accent",
      sub: "そのサイトにしかない処理",
      points: ["非公開データを使うツール", "ログインが必要な処理", "外部連携に依存するツール"],
    },
  ]}
/>
...
<FigureBars
  title="生成UIと上位検索結果の勝率比較"
  caption="Googleの論文の数字（Search Engine Journalの記事より引用）。AI Overviewの月間利用者は25億人を超えている"
  unit="%"
  bars={[
    { label: "主要プロンプトで生成UIが上位検索結果に勝った割合", value: 90 },
    { label: "情報探索系の質問での生成UIの勝率", value: 73.5 },
    { label: "情報探索系の質問での上位検索結果の勝率", value: 19 },
  ]}
/>
...
<FigureQuote text="専門家が作ったサイトが最上位、生成UIがそれに僅差で続く" source="Google Researchの論文" />
...
<FigureDoDont
  title="ツールページを持つサイトがやること・やらなくていいこと"
  dos={[
