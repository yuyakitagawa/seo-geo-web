...
## マス目を削除した（2026-09-14）
本番で実物を見たところ、**意味合いが出せなかった**ので削除した。

本文リンクだけに絞っても、出てくるのは「対角線（同じ階層の中のリンク）が太い」「その他が大きい」
という、直す判断に繋がらない形だった。第1階層でまとめた時点で粒度が粗すぎ、
「どの記事からどのページへ送るべきか」までは降りられない。

残した指摘（孤立ページ・本文から案内されていないページ・被リンクが薄いページ・リンク切れ）は
**URLが名指しで出る**ので、そのまま直せる。図に頼らず、そちらで十分だった。
`sections` / `sectionLinks` / `SectionMatrix` とテストは削除（コメントアウトで残さない）。
