...
- [x] D-1. 用語集 `/glossary`（41語 / 5分野 / DefinedTermSet + DefinedTerm）
- [ ] D-2. 日本語コピースキル（`.claude/skills/jp-copy` 相当）
- [ ] D-3. サイト共通の `/faq` —— **保留**。`/seo` `/geo` `/learn/*` `/about` `/tools/*` に既にFAQがあり、
      同じ文面を集めると自分でカニバリを作る（B-1 で対策を入れたばかりの問題そのもの）。
      作るなら「そこにしか無い質問」（サイトの使い方・データについて）に限定する。
