...
## 実装（コード側・完了）
- [x] 日次生成を下書き保存に変更し、人が一次情報と照合してから公開する運用にする
- [x] 運営者情報・免責事項・READMEを新しい公開フローに合わせる
- [x] 自動公開へ戻る変更をCIで検知する
...
## 残るリスク（コードで解決できない）
- 2026-09-11以降に生成する記事は人の確認前に公開されない。既存記事については、審査前に運営者が未確認分を通読し、
  事実の誤りと重複記事を落としておくのが安全（確認記録は `docs/progress_factcheck.md`）。
- 公開記事はニュース要約中心のため、独自の切り口（日本のサイト運営者向けの打ち手）が
