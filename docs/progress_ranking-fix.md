...
## 追記（2026-09-11）: main への載せ替えと冪等性の修正
- main（#50〜#56、ページが `src/app/(ja)/` に移動）へ rebase。衝突は CLAUDE.md の1か所（`src/app/` の行は上流、`pick.ts` の行はこちら）
- **`npm run links` が冪等でなかった**。既存リンクを勘定に入れておらず、2回目の実行で用語集リンクが2本目に増え、
  リンクのある段落に2本目が入った（28本に29本）。既存リンクを用語集の上限・段落の占有・リンク先の重複に数えるよう直し、
  テストを3本追加（90本）。2回目の実行が0本になることを確認
- 新しく公開された記事（0074 / 0075）は対象語が無く0本。リンクの無い記事は7本
- `typecheck` が `.next/dev/types/validator.ts` の古い参照（移動前の `src/app/learn/...`）で落ちたので `.next/dev` を削除（gitignore対象の生成物）
