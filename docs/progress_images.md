...
- OGP画像のビルドにはネットワークが必要（Google Fontsのサブセット取得）。
  3回試して失敗したら `ogFontOption` が `fonts` を省き、next/og 同梱の Geist で画像を生成する（和文が欠けるだけ）。

## 追記（キービジュアル廃止）
生成SVGのキービジュアルは運営判断で全廃した。`src/lib/coverArt.ts` と
`src/components/CoverArt.tsx` を削除し、記事カード・記事ページ冒頭・`PageHeader` の
画像枠を落とした。OGP画像は残し、背景をカテゴリ色のグラデーションに置き換えている。
本文中のビジュアルは `figures.tsx` の図解だけ。
