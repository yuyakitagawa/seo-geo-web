...
- **見出しと本文の噛み合い**（`src/lib/headingFit.ts`）: 見出しごとに、その下の本文と噛み合っているかを見る。
  ページ診断の指摘 `heading-fit` として出るので、サイト修正提案書にも束ねられて載る。
  **判定の主軸は「見出しの重要語が本文に出てくるか」**。見出しと本文の近さ（TF-IDFのコサイン類似度）も
  計算して結果に載せるが、**判定には使わない**。「近さ0.12」と言われても直しようがないが、
  「見出しの語が本文に1つも無い」なら直し方が一意に決まるため。廃止した `/tools/prompt-fit` が
  ...
  **誤検知を出さないための除外**: 「まとめ」「はじめに」などの定型の見出し（`BOILERPLATE`）と、
  ...
  解析の部品は `promptFit.ts` と共有する（判定を2か所に書かない）。
