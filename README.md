...
- **サイト修正提案書 `/tools/site-report`**（2026-09-13 追加）: サイトのURLを1つ入れると、代表ページを最大8本取得して検査し、
  ...
  **リンク構造**（`src/lib/linkGraph.ts`）は、フォームの「リンク構造も調べる」を入れたときだけ調べる（既定はオフ）。
  入口から内部リンクを**幅優先で最大300ページ**たどり、クリック深度の分布・どこからもリンクされていないページ・
...
- **見出しと本文の対応**（`src/lib/headingFit.ts`）: 見出しごとに、その下の本文を2つの軸で見る。
  どちらも**直し方が一意に決まるか**で選んでいる。
  1. **噛み合い**（指摘 `heading-fit`）: 見出しの重要語が本文に出てくるか。出てこない＝見出しと中身が別のことを指している。
     → 直し方は「見出しを本文に合わせて書き直す」
  2. **答えとして適切か**（指摘 `heading-answer`）: 見出しを「問い」として読み、その節が答えになっているか。
     判定は `promptFit.ts` の `detectIntent` / `formatChecks` を共有する（見出しは問いなので、プロンプト用の判定がそのまま使える）。
     **3つとも直し方が一意に決まるものだけ**にした。
     - **答えが無い**（`none`）: 聞いていることの形が本文に無い（費用の見出しに金額が無い等）→ その形を書く
     - **言い切っていない**（`hedge`）: 書き出しが「会社によって様々です」「一概には言えません」で、答えの形も無い（`HEDGE`）
       → 条件を付けてでも言い切る。**埋め込みベクトルでは見出しと同じ話題として高く出てしまう文が、語句でなら確実に捕まる**
     - **答えが後ろにある**（`late`）: 答えはあるが節の書き出し（先頭`LEAD_SENTENCES`文）に無い → 答えを先頭に出す。
       AI検索は見出しの直後から抜き出すため。ただし **howto（番号付きの手順）と compare（表）では位置を判定しない**（`POSITION_CHECKED`）。
       ブロックからは「リストや表があるか」しか分からず**どこにあるかが分からない**ので、分からないことを断定しない

  **意味ベクトル（埋め込み）は使わない**。埋め込みのコサインが測るのは「同じ話題か」であって「答えがあるか」ではない。
  「GEOの費用は会社によって様々です」は見出しと同じ話題なので高く出るが、答えていない。
  近さ（文字bigramのTF-IDFのコサイン類似度）は結果に載せるが**判定には使わない**。「近さ0.12」と言われても直しようがないが、
  ...
  **誤検知を出さないための除外**: 「まとめ」「はじめに」などの定型の見出し（`BOILERPLATE`）と、
  ...
  答えの形は、見出しに**何を聞いているか分かる語が無いとき（`intent` が `other`）は判定しない**。
  「〜はいくら？」の形でなくても「料金の内訳」のような体言止めは費用の節として見る（料金の節に金額が無いのは指摘に値する）。
  **判定の内訳は `AuditResult.headings` で返し、`/tools/page-audit` の画面に全部出す**（`HeadingFitPanel`）。
  見出しごとに「何を聞いている見出しか」「どの語が本文にあるか（○△×）」「答えの形があるか」「見出しの直後の1文」「近さ（参考値）」を並べ、
  判定しなかった見出しの数と理由も書く。**測り方が見えないと直せない**ため、指摘だけでなく内訳を出す。
- **プロンプト適合度 `/tools/prompt-fit` は 2026-09-13 に廃止**し、308で `/tools/site-report` に送っている（`vercel.json`）。
  ...
  **判定本体 `src/lib/promptFit.ts` は残している**。他人のURL向けの公開ツールとしては消したが、
  自サイトのビルド済みHTMLに当てる運用スクリプト `npm run prompt-gap`（下記）と、見出しと本文の噛み合いを見る
  `src/lib/headingFit.ts` が、同じ解析の部品（見出しブロック分割・分かち書き・TF-IDF・重要語・文の分割）を使うため。
...
- **URL取得の安全策** `src/lib/fetchPage.ts`: http/https と 80/443 のみ、名前解決先がプライベート・ループバック・リンクローカルなら拒否（リダイレクトの各ホップで再検査）、
  ...
  **タイムアウトはリダイレクトを含めた合計**（`budgetMs`。既定12秒）。ホップごとに数え直すと、3回リダイレクトされた1本で
  ...
  画面から呼ぶ側は `src/lib/postJson.ts`（何も import しない葉）を通す。`api/*.ts` はどの分岐でもJSONを返すので、
  **JSONでない応答が来たら、それはAPIまで届いていないか、関数が落ちたか、間に何かが挟まっている**
  （プレビューのアクセス保護のログイン画面・タイムアウト・502）。以前は3つの画面がそれぞれ `await res.json()` を
  try で囲み、理由に関わらず「通信に失敗しました」とだけ出していたため、**画面にも報告にも原因が残らず切り分けができなかった**。
  今は本当の通信失敗（オフライン・接続断）と、JSONでない応答（HTTPステータスを文言に出す）を分けて表示する。
  API側も `api/audit.ts` は**判定本体をハンドラの中で読み込む**（`sameOrigin` / `rateLimited` で弾くリクエストでは
