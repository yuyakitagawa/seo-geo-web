...
- **見出しと本文の対応**（`src/lib/headingFit.ts`）: 見出しごとに、その下の本文を2つの軸で見る。
  ...
  2. **答えの形**（指摘 `heading-answer`）: 見出しを「問い」として読み、聞いていること（費用・手順・理由・比較・事例・定義・判断）の
     答えの形が本文にあるか。費用の見出しに金額が無い、手順の見出しに番号付きの手順が無い、理由の見出しに「〜のため」の文が無い、など。
     → 直し方は「本文に答えを足す」。判定は `promptFit.ts` の `detectIntent` / `formatChecks` を共有する（見出しは問いなので、プロンプト用の判定がそのまま使える）。
