...
### ブランドの露出量（学習コーパス内の言及数）
`python3 scripts/exposure.py 日本 東京 Ahrefs ミエルカ`（報告のみ。ファイルは変更しない。依存なし・標準ライブラリだけ）

infini-gram の公開APIで、事前学習コーパス（既定は RedPajama）にその文字列が何回出てくるかを数える。
**fan-outクエリの候補に入るかどうかを決めるのは、KG登録でも構造化データでもなく、学習データ内の言及量**
（記事30・61）なので、その代理指標として使う。`--file` で1行1語のリスト、`--index` でコーパスを変えられる。

**必ず対照群（「日本」「東京」など確実に多い語）を一緒に測る。** 対照群まで小さければ、
コーパスにその言語が無いだけで、ブランドの評価にならない。

2026-09-08 実測（`v4_rpj_llama_s4`）: 日本 617,458 / 東京 86,650 / Ahrefs 48,037 / Semrush 16,404 /
楽天 3,052 / クレジットカード 1,649 / 三井住友 231 / **検索エンジン最適化 23** / ミエルカ 6 / ラッコキーワード 0。
日本語のカテゴリ語そのものが桁違いに薄い。記事30のfan-outクエリが英日混在（英語のフレームに日本語のブランド名）
だったのは、これで説明がつく。

読むときの注意:
- RedPajama は GPT / Gemini の学習データそのものではない。**公開コーパスの代理**
- 日本語はトークナイザが文字単位に割るため**部分一致を含む**（「日本」は「日本語」も数える）。
  小さい数字はむしろ過大に出るので、**桁の比較にだけ使う**
- 取得できなかった語は `0` ではなく `ERR` と出し、終了コードを1にする。**失敗を0件と混同しない**

**Common Crawl の index API（`index.commoncrawl.org`）は使わない。** 測れるのは「自社サイトが何ページ
拾われたか」であって「よそのページに何回名前が出たか」ではなく、露出量の指標にならない。加えて
連続リクエストで壊れ、`wc -l` で数えるとエラー応答の行数がそのまま件数に化ける
（2026-09-08、同一クエリで 7 と 14,166 が入れ替わった）。
