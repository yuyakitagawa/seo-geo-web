...
- 記事内検索、英語版
- **右下のフローティングチャットボット**（記事・用語集・教科書の本文だけを根拠に質問へ答える。設計済み・未実装。`docs/progress_chatbot.md`。
  検索はサーバー内の文字bigram TF-IDF、生成は `claude-haiku-4-5`、根拠が無ければAPIを呼ばない、入力は保存しない。実装前にモデル・Vercel側のAPIキー・月額上限・表示範囲をユーザーが決める）
