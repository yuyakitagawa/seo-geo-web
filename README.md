...
もう1系統の材料がChatGPTの会話JSON（開発者ツールのNetworkタブから保存。`data/` は .gitignore）。
候補URL・引用・検索クエリを1件ずつ数えて書く。記事30（候補36→引用4）、記事71（条件で候補が入れ替わる）、
記事75（Solの検索3段階）、記事81（候補236→引用23。比較メディアは候補に入って引用ゼロ）がこれにあたる。
標本が小さいうちは**率を書かず実数で書き**、観測条件（日付・モデル・アカウント数・カテゴリ）を冒頭に置く。
