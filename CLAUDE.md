...
- `src/lib/internalLinks.ts`: 記事本文への内部リンク差し込み（純関数。**既に本文にある語をリンクで包むだけで文言は変えない**。コードフェンス・JSX・見出し・既存リンク・「## 結論」の1段落目には触れない）。`scripts/internal-links.ts` がファイル入出力を持つ。新規記事は `scripts/prompt.ts` の `INTERNAL_LINK_RULES` が担当し、こちらは過去記事の補修用。`src/lib/toc.ts`: 記事の目次（MDXから見出しを拾い、rehype-slug と同じidを再現する）。`src/lib/icon.tsx`: ファビコン・アプリアイコン・Xのアイコンの図案（1か所。円形クロップ前提で四隅を空ける）。`src/lib/og.tsx`: OGP画像の共通枠（`ogFrame` / `pageOgImage`）。`opengraph-image.tsx` はセグメントごとに置き、下位ページへ引き継がれる。ただし `openGraph` を自前で書くページには引き継がれないので `images` を明示する。
