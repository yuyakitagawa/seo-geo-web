...
- `src/app/`: ルート。ルートレイアウトが2つある（`<html lang>` を日英で分けるため）。`(ja)/` に日本語の全ページ（`layout.tsx`, `articles/[slug]`, `news`, `seo`, `geo`, `tag/[tag]`, `about`, `privacy`, `disclaimer`, `tools/page-audit`, `tools/prompt-fit`, `glossary`）、`(en)/en/` に英語版（`/en` と `/en/articles/[slug]`）。`(ja)` `(en)` はURLに出ない。言語に依らないもの（`sitemap.ts`, `robots.ts`, `feed.xml`, `llms.txt`, `ads.txt`, アイコン, `globals.css`）は `src/app/` 直下。Organization の JSON-LD は両レイアウト共通で `src/lib/organization.ts`。
