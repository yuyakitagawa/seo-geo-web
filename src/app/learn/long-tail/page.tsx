  ...
  return (
    <LessonShell lesson={lesson} toc={TOC}>
      <GuideSection
        id="shape"
        title="検索需要のかたち"
        lead="検索されている語は、検索数の多い少数のクエリと、1件あたりは小さい膨大な数のクエリに分かれます。後者は一般に「ロングテールキーワード」と呼ばれます。前者を狙うのが一般的なSEOのイメージですが、評価が積み上がっていないサイトが最初に取れるのは後者です。ここでは後者を「細かいクエリ」と呼び、それを1本ずつではなく面で押さえる設計を扱います。"
      >
        <GuideTable
          head={["", "太いクエリ", "細かいクエリ"]}
          rows={[
      ...
      <GuideSection
        id="steps"
        title="尾を拾う手順"
        lead="ロングテールキーワードの探し方は、軸を頭の中で作るより、すでにGoogleが自分のサイトに割り当てているクエリから拾うほうが早く当たります。表示回数の多い順ではなく、少ない側を見るのがこのレッスンの手順です。"
      >
        <FigureFlow
