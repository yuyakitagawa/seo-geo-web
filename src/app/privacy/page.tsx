...
export default function PrivacyPage() {
  // 章番号。フォームの転送先が未設定の環境では「お問い合わせフォーム」の章ごと消えるため、
  // 番号は固定で書かずに上から順に振る（番号が飛ぶと本文の参照とずれる）。
  let n = 0;
  const sec = () => ++n;
    ...
    <>
      ...
      <div className={PROSE.page}>
        ...
        <p>
          ...
          本ポリシーに関するお問い合わせは
          {HAS_CONTACT_PAGE ? <><Link href="/contact">お問い合わせ</Link>のページに記載の窓口</> : "当サイトの問い合わせ窓口"}
          までお願いします。
        </p>

        <h2>{sec()}. 取得する情報</h2>
        <p>当サイトが取得するのは次の情報です。住所・電話番号などの入力を求めることはありません。</p>
        <ul>
          ...
          <li>当サイトのツールに入力されたページのURL（詳細は「ツールに入力されたURLの記録」の章に記載します）</li>
        </ul>

        <h2>{sec()}. アクセス解析ツールについて</h2>
        <p>
        ...
        {CONTACT_FORM_ENABLED && (
          <>
            <h2>{sec()}. お問い合わせフォームについて</h2>
            <p>
              <Link href="/contact">お問い合わせ</Link>のフォームから送信された内容（用件・お名前・メールアドレス・本文）は、
              当サイトのサーバーやデータベースには保存せず、運営者へ通知として転送するだけです。転送先は
              {CONTACT_MAIL_ENABLED && "運営者のメール"}
              {CONTACT_MAIL_ENABLED && CONTACT_LINE_ENABLED && "と"}
              {CONTACT_LINE_ENABLED && (
                <>
                  <a href="https://line.me/ja/terms/policy/" target="_blank" rel="noopener">LINE</a>
                  （LINEヤフー株式会社のメッセージ配信API）
                </>
              )}
              で、運営者本人だけが受け取ります。転送された内容は返信と内容の確認のためにのみ利用し、対応が完了した時点で削除します。
              連続送信を防ぐためにIPアドレスを一時的に数え上げますが、記録には残しません。
            </p>
          </>
        )}

        <h2>{sec()}. ツールに入力されたURLの記録</h2>
        <p>
          当サイトの<Link href="/tools/page-audit">ページ診断</Link>にURLを入力して検査を実行した場合、
          検査対象ページの<strong>ホスト名とパス</strong>（例: <code>example.com/blog/1</code>）と、
          判定結果の件数・HTTPステータスを記録します。どのようなページが検査されているかを把握し、
          当サイトで扱う記事の題材を選ぶために利用します。記録の内容を公開したり、第三者に提供したりすることはありません。
        </p>
        <p>次の情報は記録していません。</p>
        <ul>
          <li>URLのクエリ文字列（<code>?</code> 以降。認証トークン等が含まれることがあるため保存しません）</li>
          <li>検査を実行した方のIPアドレス・ブラウザの情報（連続実行を防ぐための一時的な回数制限にのみ使用し、記録には残しません）</li>
