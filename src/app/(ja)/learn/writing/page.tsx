  ...
  return (
    <LessonShell lesson={lesson} toc={TOC}>
      ...
      >
        ...
        <h3>条件が並ぶページは、冒頭に要点ボックスを置く</h3>
        <p>
          申込期限・費用・所要時間・必要なものといった<strong>条件が並ぶページ</strong>（手続き・料金・申し込み）では、
          セクションごとの直答に加えて、h1の直下にそのページの要点だけを並べたボックスを置きます。
          読者が知りたいのは条件の一覧であり、それが本文のあちこちに散っていると、
          同じ条件をまとめ直した第三者のページのほうが抜き出しやすい形になります。
        </p>
        <GuideTable
          head={["項目", "書き方", "ありがちな形"]}
          rows={[
            ["期限", "「作業希望日の前日まで」と、基準になる日から書く", "「お早めにお申し込みください」"],
            ["費用", "金額と、無料になる条件をその場に書く", "料金表のPDFへのリンクだけ"],
            ["所要時間", "「約20〜30分」と幅で書く", "記載が無い"],
            ["必要なもの・条件", "立ち会いの要否、必要な書類の種類", "本文の後半に散っている"],
            ["対応日", "土日祝を含むか、休業日はいつか", "営業カレンダーの画像"],
          ]}
          caption={
            <>
              項目の並べ方は当サイトの整理です。先頭に置くのは要点だけで、詳しい手順はその下に続けます
              （手順そのものを機械可読にするHowToは<Link href={`${lessonPath("technical")}#structured`}>レッスン{lessonNo("technical")}</Link>）。
            </>
          }
        />
        <p>
          要点ボックスの中身は、<strong>HTMLのテキストで書きます</strong>。料金表を画像で貼る、条件をPDFにだけ書く、
          手続きの流れをスクリーンショットで示す——この3つは、人には読めても本文としては抜き出せません。
          抜き出される単位が次の節で扱うパッセージ、つまり本文のテキストだからです。
          PDFや図を併置するのは構いませんが、<strong>数値と条件の正はHTML側に置きます</strong>。これは当サイトの整理です。
        </p>
      </GuideSection>
