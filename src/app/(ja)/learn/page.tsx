    ...
    <>
      ...
      <div className={cx(PROSE.page, "prose-headings:scroll-mt-24")}>
        ...
        <section id="plan" className="scroll-mt-24">
          ...
          <h3>直す候補が大量に出たときの並べ方</h3>
          <p>
            ページ数の多いサイトを点検すると、直す候補は数十件単位で出ます。90日の流れは同じですが、
            出てきた順に上から直すと、後から「どれが効いたのか」も「本当に直ったのか」も言えなくなります。
            候補は次の3段に分け、<strong>下から順に</strong>片付けます。
          </p>
          <FigureStack
            title="修正候補を3段に分ける"
            layers={[
              {
                ...
                tone: "geo",
                note: "3か月〜",
                desc: "著者・更新日の明示、サブドメイン間の内部リンク、独自データ。下の2段が済んでから効き始める。",
              },
              {
                label: "2段目：見え方",
                tone: "seo",
              ...
              },
              {
                ...
                tone: "accent",
                note: "〜1か月",
                desc: "重複URL・リダイレクト・noindex・サイトマップ。ここが済むまで、上の修正の効果は複数のURLに分散する。",
              },
            ]}
            baseNote="優先度は「影響の大きさ」ではなく「他の修正の前提になっているか」で決める。分け方は当サイトの整理。"
          />
          <p>
            1件ごとの書き方も決めておきます。「canonicalを直す」とだけ書いた候補は、直したかどうかを後から確認できません。
            1件につき次の6項目をそろえると、そのまま着手と検証の単位になります。
          </p>
          <GuideTable
            head={["項目", "書く内容", "無いとどうなるか"]}
            rows={[
              ["症状", "検索結果・Search Console・ログで観測できた事実だけを書く", "直す前の状態が残らず、直ったかどうかを比べられない"],
              ["対象の範囲", "1本のURLか、テンプレート単位か、ホスト全体か", "テンプレート由来の指摘を1ページだけ直して終わる"],
              ["原因", "設定・テンプレート・本文のどこに由来するか", "同じ症状が別のページで再発する"],
              ["直した後の状態", "置き換える設定や文面を、そのまま入れられる形で", "実装者が解釈し直す分だけ内容がずれる"],
              ["検証指標と時期", "何がどうなったら直ったと見るか（例：非wwwの登録数が0、URL検査で正規URLが代表）", "「対応済み」という記録だけが残る"],
              ["優先度", "上の3段のどれか", "着手順が、見つけた順や直しやすい順になる"],
            ]}
            caption={
              <>
          ...
          />
          <p>
            1段目の修正は<strong>まとめて一度に入れて構いません</strong>。
            <Link href={lessonPath("measurement")}>レッスン{lessonNo("measurement")}</Link>の「同時に複数の施策を打たない」は、
            表示回数やクリック数で効果を測りたい施策についての原則です。重複URLの一本化のように、
            検証指標が「重複が消えたかどうか」で、観測すれば直接確認できる修正は、まとめたほうが速く、互いの効果を打ち消しません。
            2段目からは1施策ずつ、4週間の窓で見ます。
          </p>
        </section>
        ...
        <section id="who" className="scroll-mt-24">
          ...
          <GuideTable
            ...
            rows={[
              ...
              [
                "ページ数が多く、直す候補が大量に出ている",
                `レッスン${lessonNo("technical")} → ${lessonNo("domain")} → ${lessonNo("updates-risk")}`,
                "重複URLとホスト名の一本化が済むまで、他の修正の効果が複数のURLに分散するため",
              ],
            ]}
          />
        </section>
        ...
        <GuideCrossLinks
          links={[
            ...
            { href: "/tools", label: "SEO・GEO診断ツール", note: "ページ診断・AIクローラー確認など、教科書と一緒に使うツール。" },
