  ...
  return (
    <LessonShell lesson={lesson} toc={TOC}>
      ...
      >
        ...
        <h3>移転していなくても、旧URLは溜まる</h3>
        <p>
          長く運用しているサイトでは、リニューアルをしていなくても旧世代のURLが現行ページと並んでインデックスされます。
          以前のCMSが出力した <code>.html</code>、連番だけのファイル名、古いタイトル書式のページなどです。
          移転と違って<strong>「移行前の一覧」が存在しない</strong>のがこの状態の厄介な点で、
          対応表を作る前に棚卸しが必要になります。材料はSearch Consoleのインデックス登録レポート（登録済みURLの一覧）と
          サーバーのアクセスログで、この2つを突き合わせて次の3つに分けます。
        </p>
        <GuideTable
          head={["分類", "見分け方", "処理"]}
          rows={[
            [
              "現行ページに置き換わっている",
              "同じ内容の新しいページがあり、旧URLはその古い版",
              "現行ページへ301。内部リンクとサイトマップも現行URLに直す",
            ],
            [
              "まだ役割がある",
              "過去の発表・資料として参照され、流入も残っている",
              "残す。タイトルと説明文を現行の書式にそろえ、canonicalを自分自身に向ける",
            ],
            [
              "役割が終わっている",
              "対応する現行ページも参照価値も無く、内部リンクも無い",
              "404を返す。内部リンクとサイトマップから外し、無関係なページへ301で寄せない",
            ],
          ]}
          caption={
            <>
              分類は当サイトの整理です。処理の前提になる考え方（内容が対応する個別ページへ転送し、対応先が無いページはトップへ寄せない）は、
              Googleが公開しているサイト移転の手順と同じです。
              <GuideRef {...REF.siteMove} />
            </>
          }
        />
        <p>
          棚卸しで見落としやすいのが、<strong>旧URLから現行URLへの経路が多段になっている</strong>ケースです。
          旧ドメイン → 旧URL → 現行URLのように途中に中間のリダイレクトが残っていると、
          経路が長いぶん取得の失敗やパラメータの欠落が起きやすくなります。対応表を作るときは、
          旧URLの行き先を<strong>1ホップで現行URLに書き換えます</strong>（中間のリダイレクト自体は残しても構いません）。
          タイトルの書式も棚卸しの対象です。同じサイトの中で古い書式と現行の書式が混在していると、
          検索結果でのブランドの見え方がページごとに変わります。
        </p>
        <p>
