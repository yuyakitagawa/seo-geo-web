...
const FAQ: FaqItem[] = [
  ...
  },
  {
    ...
    answer: `フォームの「リンク構造も調べる」にチェックを入れると調べます。入口のページから内部リンクを最大${CRAWL_MAX_PAGES}ページたどり、入口から何クリックで届くかの分布、**どの階層からどの階層へリンクしているかのマス目**、どこからもリンクされていないページ、本文（ナビ・フッターの外）から1本も案内されていないページ、サイト内から1本以下しかリンクされていないページ、200以外を返す内部リンクを出します。取得するページが増えるので、サイトの規模と表示の速さによって10秒〜1分ほどかかります（表示の速いサイトなら${CRAWL_MAX_PAGES}ページでも20秒前後で終わります）。チェックを入れないときは今までどおり最大${MAX_PAGES}ページで、20〜40秒です。`,
  },
  {
