...
const FAQ: FaqItem[] = [
  ...
  {
    question: "「人が見るページと、AIが受け取るページ」は何を比べていますか",
    answer:
      "取得した1本のHTMLから、同じURLでも人とAIで中身が変わるところだけを抜き出しています。AI検索のクローラーの多くはJavaScriptを実行せず、画面も見ないので、alt の無い画像の中の文字、iframeや動画の中身、JavaScriptを実行してから描画される部分は届きません。逆に、タブを開くまで画面に出ない文章、読み上げ用のテキスト、aria-label、JSON-LD、meta description は、画面に出なくてもそのまま届きます。ブラウザでの表示は再現していないので、HTMLから確実に分かることだけを書いています。",
  },
  {
    question: "「判定対象外」と出る項目があるのはなぜですか",
    answer:
      "その項目を判定する前提が揃っていないためです。本文が短いページや robots.txt が取れないサイトのほか、ページの種類によって「そもそも入れるべきでない」項目も対象外にします。たとえば、記事の一覧・利用規約・お問い合わせフォームのページには、質問と回答の節も、他所の原文の引用も置きません。運営者（Organization）の構造化データは、Googleがトップまたは組織を説明するページ1枚に置けばよいと書いているので、その2種類のページでだけ判定します。公開日・更新日は記事系のページでだけ判定します。",
  },
  {
    question: "「見出しと本文が噛み合っていない」はどう判定していますか",
    answer:
      "見出しから重要語を取り出し、その語が同じ節の本文に出てくるかを見ています。1つも出てこない見出しだけを指摘します。見出しと本文の「近さ」（TF-IDFのコサイン類似度）も計算していますが、判定には使っていません。近さの数値を見せられても直しようがないためです。「まとめ」「はじめに」のような定型の見出しと、本文が短い節は判定しません（語が重ならないのが当たり前で、直す必要のない指摘が並ぶため）。AI検索は見出しごとのまとまりを抜き出して回答に使うので、見出しが中身を言い当てていないと、その見出しで拾われても答えになりません。",
  },
  {
    ...
    <>
      ...
      <div className={cx(CONTAINER.wide, "space-y-14 py-14 sm:py-20")}>
        <p className="leading-relaxed text-mute">
          ...
          検査した項目は○×の一覧で返し、あわせて「人が見るページ」と「AIクローラーが受け取るページ」の違いを左右に並べます。
          100点満点のような点数は出しません。直す順番が分かることを目的にしています。
        </p>
