...
// 要素は <aside>。中身は他ページへのリンクなので、ここを本文として抽出されると
// 「SEO対策とは」の答えが記事タイトルの寄せ集めで薄まる（判定は npm run prompt-gap）。
export default function CategoryArticles({ category }: { category: CategoryKey }) {
  ...
  return (
    <aside aria-label={`${label}の記事一覧`} className={cx(CONTAINER.page, "mt-16 space-y-14")}>
      {howto.length > 0 && (
        <section>
