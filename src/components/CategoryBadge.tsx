...
import { CATEGORIES, categoryHref, type CategoryKey } from "@/lib/site";
...
export default function CategoryBadge({ category, size = "sm", asLink = true }: { category: CategoryKey; size?: "sm" | "md"; asLink?: boolean }) {
  const className = cx(BADGE[size], CATEGORY_STYLE[category].badge);
  if (!asLink) return <span className={className}>{CATEGORIES[category].label}</span>;
  return <Link href={categoryHref(category)} className={className}>{CATEGORIES[category].label}</Link>;
}
