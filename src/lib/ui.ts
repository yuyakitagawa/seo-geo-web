...
/** 入力欄。input=1行（URL・コード） / plain=1行（日本語） / area=複数行（コード・URL） / text=複数行（日本語の文章） */
export const FIELD = {
  input: "rounded-full border border-line-strong bg-canvas px-5 py-3 font-mono text-sm outline-none focus:border-accent",
  plain: "rounded-full border border-line-strong bg-canvas px-5 py-3 text-sm outline-none focus:border-accent",
