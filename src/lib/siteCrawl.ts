...
/**
 * クロールするページ数の上限。
 * **2026-09-14に実測**: 静的配信のサイト（seo-geo-lab.com）で80ページを約5秒。1ページあたり実質0.06秒で、
 * 期限（CRAWL_DEADLINE_MS=50秒）にはまるで届かなかったため 80 → 300 に上げた。
 * 遅いサイトは上限ではなく期限で止まる（取れた分で提案書を作る）ので、上げてもタイムアウトのリスクは増えない。
 * 300にすると中小規模のサイトなら全ページ辿り切れることが多く、「どこからもリンクされていない」を
 * 候補ではなく**断定**して出せるようになる（linkGraph.ts の truncated が false になる）。
 */
export const CRAWL_MAX_PAGES = 300;
