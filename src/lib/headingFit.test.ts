...
test("BudouX が切り損ねた欠片を語として出さない", () => {
  // 「なぜAI検索に…」は「な」「ぜAI検索に」と切られることがある。「ぜai検索」は語ではない
  const terms = keyTerms("なぜAI検索に引用されないのか").map((t) => t.term);
  assert.ok(
    !terms.some((t) => /^[ぁ-ゟ][^ぁ-ゟ]/.test(t)),
    `ひらがな＋別の文字種で始まる欠片が混ざっている: ${terms.join("/")}`,
  );
  // ひらがなだけの語は残す（「いくら」は答えの形の判定に効く）
  assert.ok(keyTerms("費用はいくらですか").map((t) => t.term).includes("いくら"));
});
