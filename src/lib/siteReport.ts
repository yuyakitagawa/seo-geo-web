...
const RULES: Record<string, Rule> = {
  ...
  "heading-fit": {
    stage: 2,
    origin: "content",
    cause: "見出しを先に決めて本文を書いた（または本文を書き換えたときに見出しを直していない）。テンプレートの決め打ちの見出しをそのまま使っている場合もある。",
    metric: "見出しの語が本文に出てくる。修正後すぐ確認できる（AI検索での引用の変化は3か月）。",
  },
  "heading-answer": {
    stage: 2,
    origin: "content",
    cause: "見出しを問いの形にした一方で、本文が一般論の説明のままになっている。答え（金額・手順・理由）を節の先頭に置く書き方になっていない。",
    metric: "見出しが聞いていることの答えの形が、その節の本文にある。修正後すぐ確認できる（AI検索での引用の変化は3か月）。",
  },
  faq: {
    stage: 2,
    origin: "content",
